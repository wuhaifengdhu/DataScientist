...
class ProfileHelper(object):
    ...
    def generate_vector(profile_column):
        vector_dict = {'skills': ProfileHelper.generate_skills(profile_column[3]),
                       'university': ProfileHelper.generate_education(profile_column[5])[0],
                       'education': ProfileHelper.generate_education(profile_column[5])[1],
                       'major': ProfileHelper.generate_education(profile_column[5])[2],
                       'years': ProfileHelper.generate_years_company(profile_column[6])[0],
                       'company': ProfileHelper.generate_years_company(profile_column[6])[1]}
        return vector_dict
    ...
    def generate_education(education_content):
        ...
        for education in str(education_content).split('///'):
            ...
            if len(education_detail) < 2:
                ...
                continue
            university_list.append(education_detail[0])
        ...
        return university_list, education_list, major_list

    @staticmethod
    def generate_years_company(works_content):
        year_list = []
        company_list = []
        ...
        for work in work_list:
            ...
            if len(work_detail) < 3:
                ...
                continue
            company_list.append(work_detail[1])
            year_list.append(work_detail[2])
        return year_list, company_list
    ...
    def get_years(profile):
        _, years = ProfileHelper.calculate_years(profile)
        if years == 0:
            return ['[0]']
        elif years <= 2:
            return ['(0, 2]']
    ...
    @staticmethod
    def get_discipline(major_phrase, prefer_index, discipline_phrase_dic, debug=False):
    ...
    @staticmethod
    def extract_profile(debug=False):
    ...
    def convert_profile2(debug=False):
        ...
        for _profile in profile_vectors:
            ...
            educations, majors = ProfileHelper.get_highest_education(_profile, education_phrase_dic,
                                                                     discipline_phrase_dic)
            profile_dict = {'skills': ProfileHelper.get_skills(_profile, skills_dic),
                            'work_change_times': ProfileHelper.calculate_years(_profile)[0],
                            'years': ProfileHelper.calculate_years(_profile)[1],
                            'university': ProfileHelper.convert_university(_profile, university_name_convert_dict),
                            'education': educations,
                            'company': [SegmentHelper.normalize(company) for company in _profile['company']],
                            'major': majors}
            vector_list.append(profile_dict)
        StoreHelper.store_data(vector_list, '../resource/convert_profile.dat')
        StoreHelper.save_file(vector_list, '../resource/convert_profile.txt')
    ...
    def convert_university(profile, convert_dict, debug=False):
        ...
        if len(university_list) == 0:
            return None
        university_list = [SegmentHelper.normalize(university) for university in university_list]
...
if __name__ == '__main__':
    ...
    ProfileHelper.convert_profile2(True)
