...
class TextHelper(object):
    ...
    @staticmethod
    def company_name_clean(company_name):
        company_name = company_name.lower()
        if company_name.endswith(".com"):
    ...
    @staticmethod
    def clean_unicode_text(origin_text):
        return unicodedata.normalize('NFKD', origin_text).encode('ascii', 'ignore')

    @staticmethod
    def find_max_length_str_in_list(str_list):
        if len(str_list) == 0:
            return None
        max_length_str = str_list[0]
        for i in range(1, len(str_list)):
            if len(str_list[i]) > max_length_str:
                max_length_str = str_list[i]
...
if __name__ == '__main__':
    ...
    print (type(TextHelper.clean_unicode_text(u'UNIVERSITÌãT MANNHEIM')))