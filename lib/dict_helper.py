...
class DictHelper(object):
    ...
    @staticmethod
    def find_in_key(_dict, search_key, debug=False):
        for key in _dict:
            if DictHelper.word_in_phrase(key, search_key) or DictHelper.word_in_phrase(search_key, key):
                if debug:
                    print ("Find mapping %s --> %s" % (search_key, key))
                return key
        return None
...
if __name__ == '__main__':
    ...
    print (DictHelper.word_in_phrase('google', 'google.com'))